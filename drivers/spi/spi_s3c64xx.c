...
// VenturiGB_Usys_jypark 2011.08.08 - DMB (Ref.GalaxyS_Kor GB) [[
#ifdef DEBUGSPI
static void dump_regs(struct s3c64xx_spi_driver_data *sspi)
{
...
#define SPI_STUS_TX_FIFORDY             (1<<0)
        ...
        u32 val;
        struct s3c64xx_spi_info *sci = sspi->cntrlr_info;

        printk("SPI-%d :: ", sspi->pdev->id );
        val = readl(sspi->regs + S3C64XX_SPI_CH_CFG);
        printk("CHN-%x\t", val);
        val = readl(sspi->regs + S3C64XX_SPI_CLK_CFG);
        printk("CLK-%x\t", val);
        val = readl(sspi->regs + S3C64XX_SPI_MODE_CFG);
        printk("MOD-%x\t", val);
        val = readl(sspi->regs + S3C64XX_SPI_SLAVE_SEL);
        printk("SLVSEL-%x\t", val);
        val = readl(sspi->regs + S3C64XX_SPI_STATUS);
           printk("status=%x\n", val );
        ...
        printk("Rx/TxLvl=%d,%d \n", RX_FIFO_LVL(val, sci), TX_FIFO_LVL(val, sci));
}
...
#ifdef DEBUGSPI	// VenturiGB_Usys_jypark 2011.08.08 - DMB (Ref.GalaxyS_Kor GB) [[
		dump_regs(sdd);
#endif			// VenturiGB_Usys_jypark 2011.08.08 - DMB (Ref.GalaxyS_Kor GB) ]]
		/* Quiese the signals */
