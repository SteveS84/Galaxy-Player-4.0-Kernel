...
//extern int top_off_interrupted;
enum cable_type_t curent_device_type = CABLE_TYPE_NONE;
...
max8998_irq_table_type max8998_irq_table[ENDOFIRQ+1] = {
    ...
    NULL, // JIGR
    NULL, // JIGF
    maxim_vac_connect, // DCINR	/* use callbacks for connect */
    maxim_vac_disconnect, // DCINF	/* use callbacks for disconnect */
    NULL, // ALARM0
    NULL, // ALARM1
    NULL, // SMPLEVNT
    NULL, // WTSREVNT
    ...
    NULL, // CHGRSTF
    NULL, // DCINOVPR
    NULL, //maxim_charging_topoff, // TOPOFFR
    NULL, // ONKEY1S
    NULL, //maxim_low_battery_2nd, // LOBAT2
    NULL, //maxim_low_battery_1st  // LOBAT1
};
#endif
...
void lpm_mode_check(void)
{
	if(get_boot_charger_info())
	{
		if(maxim_lpm_chg_status()) {
			charging_mode_set(1);
			printk("(%s : charging mode booting set\n", __func__);
		}
		else{
#if 1 		/* if the microusb RESET is done, the system will be powered off.*/
	  		/* lpm mode but no cable state... we have to shutdown system in lpm mode application. so, we must set to '1' here. */
			charging_mode_set(1);
			printk("(%s : charging mode booting set - fake!!!\n", __func__);
#else
			if (pm_power_off)
				pm_power_off();
#endif
...
#if 0 /* sone-kor does not use this function */
...
/*
	...
	hrtimer_start(&pm_sys_init_timer,
					...
					HRTIMER_MODE_REL);
*/
#ifndef __USE_LPM_BOOT_IMG__
	lpm_mode_check();
#else
	if(get_boot_charger_info() && maxim_chg_status())
	{
		charging_mode_set(1);
		printk("(%s : charging mode booting set\n", __func__);
	}
	else if(get_boot_charger_info())
	{
		charging_mode_set(1); //lpm mode but no cable state... we have to shutdown system in lpm mode application. so, we must set to '1' here.
		printk("(%s : Power off cause charger removed in LPM mode - fake!!!\n", __func__);
//		if (pm_power_off)
//			pm_power_off();
	}
	else
	{
		charging_mode_set(0);
		printk("(%s : charging mode booting unset\n", __func__);
	}
	//writel(0,S5P_INFORM5); //clear
#endif /* __USE_LPM_BOOT_IMG__ */
...
{
	if(curent_device_type==CABLE_TYPE_AC)
	{
		Set_MAX8998_PM_REG(TP, 0x0); 
		printk("%s dev: TA\n",__func__);
	}
	else if (curent_device_type==CABLE_TYPE_USB)
	{
...
{
	//printk("%s\n",__func__);
	//top_off_interrupted = 1;
	if(curent_device_type != CABLE_TYPE_NONE)
		s3c_cable_charging();
	//else
...
int  FSA9480_PMIC_CP_USB(void)
{
	...
	printk(" FSA9480_PMIC_CP_USB switch_sel : %d \n",switch_sel);
...
void maxim_ta_charging_mode(int mode)
{
	...
	if(ta_mode == 1)
		reg_buff[0] = (0x0<<5) | (0x3<<3) | (0x5<<0) ; // CHG_TOPOFF_TH=10%, CHG_RST_HYS=disable, AC_FCGH= 700mA -> 600mA, requested by cy_yun
	else if(ta_mode == 2)
		reg_buff[0] = (0x0<<5) | (0x3<<3) | (0x3<<0) ; // CHG_TOPOFF_TH=10%, CHG_RST_HYS=disable, AC_FCGH= 550mA
	else
...
void maxim_charging_control(unsigned int dev_type  , unsigned int cmd)
{
	...
	else if(dev_type == CABLE_TYPE_AC)
	{
		if(ta_mode == 1)
			reg_buff[0] = (0x0<<5) | (0x3<<3) | (0x5<<0) ; // CHG_TOPOFF_TH=10%, CHG_RST_HYS=disable, AC_FCGH= 700mA -> 600mA, requested by cy_yun
		else if(ta_mode == 2)
...
void maxim_charging_control(unsigned int dev_type  , unsigned int cmd)
{
	...
	{
		//Get_MAX8998_PM_ADDR(CHGR1, reg_buff, 2);
		reg_buff[0] = (0x2<<5) | (0x1<<3) | (0x5<<0) ; // CHG_TOPOFF_TH=20%, CHG_RST_HYS=100mV, AC_FCGH= 600mA /* for test */
		//reg_buff[0] = (0x4<<5) | (0x1<<3) | (0x5<<0) ; // CHG_TOPOFF_TH=30%, CHG_RST_HYS=100mV, AC_FCGH= 600mA
		reg_buff[1] = (0x2<<6) | (0x3<<4) | (0x0<<3) | (0x0<<1) | (0x0<<0);
		Set_MAX8998_PM_ADDR(CHGR1, reg_buff, 2); 
		printk("%s TA charging enable \n",__func__);
	}
	...
	{	
		//Get_MAX8998_PM_ADDR(CHGR1, reg_buff, 2);
		reg_buff[1] = Get_MAX8998_PM_REG(CHGR2);
		printk("%s read CHGR2 = 0x%x\n", reg_buff[1]);
		reg_buff[0] = (0x2<<5) | (0x1<<3) | (0x2<<0) ; // CHG_TOPOFF_TH=20%, CHG_RST_HYS=100mV, AC_FCGH= 475mA
		//reg_buff[0] = (0x4<<5) | (0x1<<3) | (0x2<<0) ; // CHG_TOPOFF_TH=30%, CHG_RST_HYS=100mV, AC_FCGH= 475mA
		reg_buff[1] = reg_buff[1] | (0x3<<4) | (0x0<<3) | (0x0<<1) | (0x0<<0); //FCHG_TMR=disable, MBAT_REG_TH=4.2V, MBATT_THERM_REG=105C
		Set_MAX8998_PM_ADDR(CHGR1, reg_buff, 2); 
		printk("%s USB charging enable \n",__func__);
	}
	else
	{
		Set_MAX8998_PM_REG(CHGENB, 0x1); //disable charge
		printk("%s charging disable (unknown charger) \n",__func__);
	}
}
#endif
...
void maxim_vac_connect(void)
{
	...
	if(curent_device_type == CABLE_TYPE_NONE)
	{
		...
		if(UsbStatus){
			...
			curent_device_type = CABLE_TYPE_USB;
		}
		else{
			//printk("maxim_TA_connect~~~ \n");
			curent_device_type = CABLE_TYPE_AC;
		}
		s3c_cable_changed(true);
	}		
}
...
void maxim_vac_disconnect(void)
{
	//printk("vac disconnect : check charging mode\n");
	/*
	if(charging_mode_get() && !maxim_chg_status())
	{
...
#if 1
	//printk("vac disconnect : check chg & jig status\n");
	if(!maxim_chg_status()) // we have to check this. because disconnect interrupt is enabled once in booting process
	{
		printk("maxim_vac_disconnect \n");
		s3c_cable_changed(false);
		curent_device_type = CABLE_TYPE_NONE;
	}
#else /* check with batt_check() role...???*/
	printk("maxim_vac_disconnect \n");
	s3c_cable_changed(false);
	curent_device_type = CABLE_TYPE_NONE;
#endif
