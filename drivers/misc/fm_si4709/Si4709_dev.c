...
#define TUNE_RSSI_THRESHOLD     0x02 //0x00
#define TUNE_SNR_THRESHOLD      0x00 //0x01
#define TUNE_CNT_THRESHOLD      0x00 //0x00
...
typedef struct {
	/* Any function which views/modifies the fields of this structure
	 * does i2c communication should lock the mutex before doing so.
	 * Recursive locking should not be done.
	 * In this file all the exported functions will take care
	 * of this. The static functions will not deal with the	mutex
	 */
...
//static int insert_preset(u32,u8,u8*); //Not used
...
int Si4709_dev_init(struct i2c_client *client)
{
	...
	if (!RDS_Block_Data_buffer) {
		pr_err("%s: Failed to allocate RDS_Block_Data\n", __func__);
		ret = -ENOMEM;
		goto EXIT;
	}
	...
	if (!RDS_Block_Error_buffer) {
		pr_err("%s: Failed to allocate RDS_Block_Error\n", __func__);
		ret = -ENOMEM;
...
int Si4709_dev_powerup(void)
{
	...
	if (!(RADIO_ON==Si4709_dev.state.power_state)) {
		if ((ret = powerup()) < 0)
			...
			pr_err("%s: valid_client_state is eFALSE\n", __func__);
			ret = -1;
		} else {
			/* initial settings */
#ifdef _ENABLE_RDS_
...
#endif
			/* wrap at the upper and lower band limit and continue seeking */
...
#endif
			/* Setting DE-Time Constant as 50us(Europe,Japan,Australia) */
			...
			/* modified for detecting more stations of good quality */
			SYSCONFIG2_BITSET_SEEKTH(&Si4709_dev.registers[SYSCONFIG2], TUNE_RSSI_THRESHOLD);
#ifdef CONFIG_SND_SMDKC110_MC1N2
			...
			/* modified for detecting more stations of good quality */
			SYSCONFIG3_BITSET_SKSNR(&Si4709_dev.registers[SYSCONFIG3], TUNE_SNR_THRESHOLD);
		...
		pr_warn("%s: Device already Powered-ON\n", __func__);
...
int Si4709_dev_suspend(void)
{
	...
	if (Si4709_dev.valid_client_state == eFALSE) {
		pr_err("%s: valid_client_state == eFALSE\n", __func__);
		ret = -1;
	}
	mutex_unlock(&(Si4709_dev.lock));
...
int Si4709_dev_sys_config2(sys_config2 *sys_conf2)
{
	...
	if (Si4709_dev.valid == eFALSE) {
		...
		if ((ret = i2c_read(SYSCONFIG2)) < 0) {
			...
			sys_conf2->rssi_th =
				SYS_CONFIG2_RSSI_TH(Si4709_dev.registers[SYSCONFIG2]);
			sys_conf2->fm_band =
				SYS_CONFIG2_FM_BAND(Si4709_dev.registers[SYSCONFIG2]);
			sys_conf2->fm_chan_spac =
				SYS_CONFIG2_FM_CHAN_SPAC(Si4709_dev.registers[SYSCONFIG2]);
			sys_conf2->fm_vol =
				SYS_CONFIG2_FM_VOL(Si4709_dev.registers[SYSCONFIG2]);
		}
	}
	mutex_unlock(&(Si4709_dev.lock));
...
int Si4709_dev_power_config(power_config *pow_conf)
{
	...
	if (Si4709_dev.valid == eFALSE) {
		...
		if ((ret = i2c_read(POWERCFG)) < 0) {
			...
			pow_conf->dsmute =
				POWER_CONFIG_SOFTMUTE_STATUS(Si4709_dev.registers[POWERCFG]);
			pow_conf->dmute	=
				POWER_CONFIG_MUTE_STATUS(Si4709_dev.registers[POWERCFG]);
			pow_conf->mono =
				POWER_CONFIG_MONO_STATUS(Si4709_dev.registers[POWERCFG]);
			pow_conf->rds_mode =
				POWER_CONFIG_RDS_MODE_STATUS(Si4709_dev.registers[POWERCFG]);
			pow_conf->sk_mode =
				POWER_CONFIG_SKMODE_STATUS(Si4709_dev.registers[POWERCFG]);
			pow_conf->seek_up =
				POWER_CONFIG_SEEKUP_STATUS(Si4709_dev.registers[POWERCFG]);
			pow_conf->seek =
				POWER_CONFIG_SEEK_STATUS(Si4709_dev.registers[POWERCFG]);
			pow_conf->power_disable =
				POWER_CONFIG_DISABLE_STATUS(Si4709_dev.registers[POWERCFG]);
			pow_conf->power_enable =
				POWER_CONFIG_ENABLE_STATUS(Si4709_dev.registers[POWERCFG]);
		}
	}
	mutex_unlock(&(Si4709_dev.lock));
...
{
	int ret = 0;
#ifndef CONFIG_FMRADIO_CODEC_GAIN
	u16 sysconfig2 = 0;
#endif
...
#ifdef CONFIG_FMRADIO_CODEC_GAIN
#ifdef CONFIG_SND_S5P_WM8994
        *volume = wm8994_get_fm_codec_gain();
#endif
#ifdef CONFIG_SND_SMDKC110_MC1N2
        *volume = McDrv_Ctrl_get_fm_vol();
#endif
#else
	if (Si4709_dev.valid == eFALSE) {
...
/* Adding DSMUTE ON/OFF feature.The Soft Mute feature is available to attenuate 
 * the audio outputs and minimize audible noise in very weak signal conditions.
 */
	...
	if (Si4709_dev.valid == eFALSE ) {
		...
		if ((ret = i2c_read(RDSD)) < 0)
			...
			RDS_Block_Error_buffer[i++ + 4*RDS_Buffer_Index_write] =
				STATUSRSSI_RDS_BLOCK_A_ERRORS(Si4709_dev.registers[STATUSRSSI]);
			RDS_Block_Error_buffer[i++ + 4*RDS_Buffer_Index_write] =
				READCHAN_BLOCK_B_ERRORS(Si4709_dev.registers[READCHAN]);
			RDS_Block_Error_buffer[i++ + 4*RDS_Buffer_Index_write] =
				READCHAN_BLOCK_C_ERRORS(Si4709_dev.registers[READCHAN]);
			RDS_Block_Error_buffer[i++ + 4*RDS_Buffer_Index_write] =
...
static int  insert_preset(u32 frequency,u8 rssi,u8 *seek_preset_rssi)
{
	...
	/* Fill the first preset with a zero for the frequency.
	 * This will always overwrite the last preset once
	 * all presets have been filled.
	 */
...
#ifdef CONFIG_FMRADIO_CODEC_GAIN
#ifdef CONFIG_SND_SMDKC110_MC1N2
u8 fm_si4709_gain_table[FM_VOL_LEVEL] = {
        0x02, 0x02, 0x02, 0x03, 0x04, 0x05, 0x06, 0x06, 0x06, 0x06, 
        0x06, 0x06, 0x06, 0x06, 0x06, 0x06, 0x07, 0x08, 0x09, 0x0A, 
...
int McDrv_Ctrl_set_fm_vol(u8 volume)
{
        ...
        if (Si4709_dev.valid == eFALSE) {
                ...
                printk("Si4709_dev_volume_set : volume is %d\n", volume);

                SYSCONFIG3_BITSET_VOLEXT_DISB(&Si4709_dev.registers[SYSCONFIG3]);
                SYSCONFIG3_BITSET_RESERVED(&Si4709_dev.registers[SYSCONFIG3]);

                if (volume >= 0 && volume < FM_VOL_LEVEL) {
                        SYSCONFIG2_BITSET_VOLUME(&Si4709_dev.registers[SYSCONFIG2], 
                                                fm_si4709_gain_table[volume]);
                }
