...
static DEVICE_ATTR(calibrate, S_IRUGO, bma_fs_cal, NULL);
...
{	
	...
	bma222_read_accel_xyz(&acc);
//#ifdef BMA_DEBUG
	printk( "BMA Accel: X/Y/Z axis: %-8d %-8d %-8d\n" ,
...
//#endif
	...
	{
#ifdef BMA_DEBUG
	printk( "BMA Accel: copy_to_user result: %d\n", ret);
#endif
	}
	return sizeof(acc);
}
...
static ssize_t bma_write(struct file *file, const char __user *buf, size_t count, loff_t *offset)
{
	if( bma_client == NULL )
		return -1;
#ifdef BMA_DEBUG
	printk( "BMA Accel should be accessed with ioctl command\n");
#endif
	return 0;
}
...
#ifdef BMA_DEBUG
	printk( "BMA Accel has been opened\n");
#endif
	return 0;
}
...
static int bma222_fast_calibration(signed char *data)
{
...
#ifdef DEBUG
	printk( "%s\n",__func__);
    printk( "data are %d,%d,%d\n",data[0],data[1],data[2]);
    printk( "start x axis fast calibration\n");
#endif
    bma222_set_offset_target_x(data[0]);
    //bma222_get_offset_target_x(&tmp);
    //printk( "x offset is %d\n",tmp);
    //bma222_get_offset_filt_x(&tmp);
    //printk( "x offset filt is %d\n",tmp);
...
#ifdef DEBUG
        printk( "wait 2ms and got cal ready flag is %d\n",tmp);
#endif  
    }while(tmp==0);
    ...
    bma222_get_offset_filt_x(&tmp);
    printk( "x offset filt is %d\n",tmp);
    printk( "x axis fast calibration finished\n");
    printk( "start y axis fast calibration\n");
#endif

    bma222_set_offset_target_y(data[1]);
    //bma222_get_offset_target_y(&tmp);
    //printk( "y offset is %d\n",tmp);
    //bma222_get_offset_filt_y(&tmp);
    //printk( "y offset filt is %d\n",tmp);
    ...
    bma222_get_offset_filt_y(&tmp);
    printk( "y offset filt is %d\n",tmp);
    printk( "y axis fast calibration finished\n");
    printk( "start z axis fast calibration\n");
#endif
    bma222_set_offset_target_z(data[2]);

    //bma222_get_offset_target_z(&tmp);
    //printk( "z offset is %d\n",tmp);
    //bma222_get_offset_filt_z(&tmp);
    //printk( "z offset filt is %d\n",tmp);
    ...
    bma222_get_offset_filt_z(&tmp);
    printk( "z offset filt is %d\n",tmp);
    printk( "z axis fast calibration finished\n");
    printk( "store xyz offset to eeprom\n");
#endif
    tmp=1;//unlock eeprom
    bma222_set_ee_w(tmp);
...
#ifdef DEBUG
        printk( "wait 2ms and got eeprom writing status is %d\n",tmp);
#endif  
    }while(tmp==0);
    ...
    bma222_set_ee_w(tmp);
#ifdef DEBUG
    printk( "eeprom writing is finished\n");
#endif  
    return 0;
}
#endif
...
printk( "%s\n",__func__); 
	...
	case BMA222_READ_ACCEL_XYZ:
		if(sensor_type == BMA222)
		{
			...
			if(copy_to_user((struct bma222acc_t*)arg,(bma222acc_t*)data,3*sizeof(short))!=0)
			{
#ifdef BMA_DEBUG
...
{
	bma222acc_t acc,read_acc;
	int err;
		
	err = bma222_read_accel_xyz(&read_acc);
			 ...
			 const struct i2c_device_id *id)
{
	...
	// [HSS] Support both address ( 0X18(old) and 0X08(new) ) (2010.09.29)
      if( tempvalue <0)
      {
         if(bma222_i2c_slave_addr == BMA222_I2C_ADDR)
         {
            ...
            printk( "[HSS][%s] Slave Address is not matched, so, it will retry i2c transfer with changed slave address!\n", __func__);
		    tempvalue = i2c_smbus_read_word_data(client, 0x00);
         }
      }
#else
	...
	printk( "bma accel device create ok\n");
