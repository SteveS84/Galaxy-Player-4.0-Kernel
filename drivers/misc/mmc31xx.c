...
int mmc31xx_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct regulator *sensor_core_regulator;
	unsigned char data[16] = {0};
	int res = 0;

	printk(KERN_ERR "[mmc31xx_probe] +\n");

	sensor_core_regulator = regulator_get(NULL, "vled");
	res=regulator_enable(sensor_core_regulator);
	
	printk(KERN_ERR "[mmc31xx_probe] LDO %d\n", res);
