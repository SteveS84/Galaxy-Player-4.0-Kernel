...
static unsigned long freflash_lux_value = 0;

#ifdef USE_BURSTMODE	
unsigned char buf_for_burstmode[10000];
#endif
...
#ifdef S5K5CCGX_FLASH_SUPPORT
static int s5k5ccgx_set_flash(int flash_func, struct v4l2_subdev *sd);
							...
							int index, char *name)
{
	...
	else
	{
		for (i=0 ; i <index; i++) {
			...
			if (unlikely(err < 0)) {
				v4l_info(client, "%s: register set failed(BURST)\n", \
				__func__);
				return err;
			}
		}
	...
	struct i2c_msg msg = { client->addr, 0, 0, buf_for_burstmode };
	...
	for (i = 0; i < index; i++) 
	{
		switch( regs[i].addr )
		{
                     case 0x0F12 :
                         ...
                         if(idx ==0) 	
         		 	{
         		 		buf_for_burstmode[idx++] = 0x0F;
         		 		buf_for_burstmode[idx++] = 0x12;
         			 }
                         	buf_for_burstmode[idx++] = regs[i].val>> 8;
     		         	buf_for_burstmode[idx++] = regs[i].val & 0xFF;
     		         //write in burstmode	
                         if(regs[i+1].addr != 0x0F12)
                         {
	...
	if (index !=0) {
		if (unlikely(err < 0))
		{
			v4l_info(client, "%s: register set failed(NORMAL)\n", \
					__func__);
			return err;
		}
	}
...
static int s5k5ccgx_set_flash_mode(struct v4l2_subdev *sd, int flash_brightness_value, bool value)
{
	...
	printk("%s: value(%d),flash_brightness_value(%d),flash_modescene_mode(%d) \n", __func__, value, flash_brightness_value ,flash_mode);
	...
	{
		s5k5ccgx_set_flash(FLASH_OFF, sd);
		state->flashstate = FALSE;
	}
	
	printk("%s: done: current flash mode = %d\n", __func__, state->flashstate);
	return 0;
}
...
static int s5k5ccgx_set_flash_start_end(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	printk("%s: ctrl->value = %d, flash_mode =%d, flash_state = %d \n", __func__, ctrl->value, flash_mode, state->flashstate);

	switch(ctrl->value)
	{
		case FREFLASH_START:
			// check illuminatioin
			freflash_lux_value = s5k5ccgx_get_illumination(sd);
			// determin flash on/off
			if(flash_mode == FLASHMODE_AUTO)
			{
				if (freflash_lux_value < 0x0020) 
				{
					printk("%s: freflash_lux = %x\n", __func__, freflash_lux_value);
					state->flashstate = TRUE;
				}
				 else
				 {
			...
			// set before preflash on
			if (state->flashstate == TRUE)
			{
				printk("%s: state->flashstate = %d\n", __func__, state->flashstate);
				...
				// flash on
				err = s5k5ccgx_set_flash(FLASH_IN_AF_ON, sd);
				msleep(700);
				//wait for AE stable
				for(count=0; count <4; count++)
				{
					...
					err=s5k5ccgx_i2c_write(client,0xFCFC, 0xD000);
					err=s5k5ccgx_i2c_write(client,0x002C, 0x7000);
					err=s5k5ccgx_i2c_write(client,0x002E, 0x2A70);
					err=s5k5ccgx_i2c_read(client, 0x0F12, &read_value);
		...
		case MOVIEFLASH_START:
			// determin flash on/off
			// VENTURI EUR Project(taejin.hyeon) 2011.02.18.
			// Venturi eur not support auto flash in camcording
			/*
			if(flash_mode == FLASHMODE_AUTO)
			{
				lux_value = s5k5ccgx_get_illumination(sd);
			...
			*/
			...
			if (state->flashstate == TRUE)
			{	
				err = s5k5ccgx_set_flash(TORCH_ON, sd);
			}
		break;
	...
	printk("%s: done: flashmode = %d, flashstate = %d\n", __func__, flash_mode, state->flashstate);
...
static int s5k5ccgx_set_touch_auto_focus(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	unsigned short first_x, first_y, second_x, second_y;
	...
	s5k5ccgx_short_t S5K5CCGX_TOUCH_AF[]=
	{
	{0xFCFC, 0xD000},
	{0x0028, 0x7000},
	...
	unsigned int s5k5ccgx_len_set_touch_af = 12;
	...
	{
		first_x = 0;
		if (x <= INNER_WINDOW_WIDTH/2)
			second_x = 0;
		else
			second_x = x - INNER_WINDOW_WIDTH/2;
	}
	else if (width - OUTER_WINDOW_WIDTH <= x)
	{
		first_x = width - OUTER_WINDOW_WIDTH;
		if (x <= width - OUTER_WINDOW_WIDTH + INNER_WINDOW_WIDTH/2)
			second_x = width - OUTER_WINDOW_WIDTH;
		else if (width - INNER_WINDOW_WIDTH <= x)
			second_x = width - INNER_WINDOW_WIDTH ;
		else
			second_x = x - INNER_WINDOW_WIDTH/2;
	}
	else
	{
		first_x = x - OUTER_WINDOW_WIDTH/2 ;
		second_x = x - INNER_WINDOW_WIDTH/2 ;
	}

	/* get start y position */
	if (y <= OUTER_WINDOW_HEIGHT/2)
	{
		first_y = 0;
		if (y <= INNER_WINDOW_HEIGHT/2)
			second_y = 0;
		else
			second_y = y - INNER_WINDOW_HEIGHT/2;
	}
	else if (height - OUTER_WINDOW_HEIGHT <= y)
	{
		first_y = height - OUTER_WINDOW_HEIGHT;
		if (y <= height - OUTER_WINDOW_HEIGHT + INNER_WINDOW_HEIGHT/2)
			second_y = height - OUTER_WINDOW_HEIGHT;
		else if (height - INNER_WINDOW_HEIGHT <= y)
			second_y = height - INNER_WINDOW_HEIGHT ;
		else
			second_y = y - INNER_WINDOW_HEIGHT/2;
	}
	else
	{
		first_y = y - OUTER_WINDOW_HEIGHT/2 ;
		second_y = y - INNER_WINDOW_HEIGHT/2 ;
	}
	...
	{
		S5K5CCGX_TOUCH_AF[3].val = first_x * 1024 / width;
		S5K5CCGX_TOUCH_AF[4].val = first_y * 1024 / height;
		S5K5CCGX_TOUCH_AF[5].val = OUTER_WINDOW_WIDTH *1024 /width;
		S5K5CCGX_TOUCH_AF[6].val = OUTER_WINDOW_HEIGHT*1024 /height;
		
		S5K5CCGX_TOUCH_AF[7].val = second_x * 1024 / width;
		S5K5CCGX_TOUCH_AF[8].val = second_y * 1024 / height;
		S5K5CCGX_TOUCH_AF[9].val = INNER_WINDOW_WIDTH *1024 /width;
	...
	{
		err = s5k5ccgx_i2c_write(client, S5K5CCGX_TOUCH_AF[i].addr, S5K5CCGX_TOUCH_AF[i].val);
	}
...
static int s5k5ccgx_get_auto_focus_status(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
	{
	int err, progress_count=0, idle_count=0;
	...
	// 1st AF search
	for(progress_count=0; progress_count <40; progress_count++)
	{
		//force cencel
		if (stop_af_operation == 1)
		{
		...
		else if(read_value == 0x0000 || read_value == 0x0008)
		{
			if (state->scenemode == SCENE_MODE_NIGHTSHOT || state->scenemode == SCENE_MODE_FIREWORKS)
			{
	...
	touch_af_status = 0; //touch af clear
	stop_af_operation = 0; //af stop clear
...
static int s5k5ccgx_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	s5k5ccgx_info(&client->dev, "%s:V4l2_control_ID:%d:ctrl->value:%d\n", __func__, ctrl->id - V4L2_CID_PRIVATE_BASE, ctrl->value);
			 ...
			 const struct i2c_device_id *id)
{
	...
	printk("s5k5ccgx has been probed\n");
	
	/* For the test */
	printk("Forced power en %s\n",__func__);
	//s5k5ccgx_power_en(1);
