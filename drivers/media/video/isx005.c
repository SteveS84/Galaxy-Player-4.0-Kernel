...
#define SENSOR_JPEG_SNAPSHOT_MEMSIZE	0x456000     //0x460000 //2960 * 1536

/* Used for DNIE firmware upgrade; nothing to do with camera */
//#define MDNIE_TUNING
...
static int camera_init = 0; /* Camera init check */
static int cdbg = 0;
static int gLowLight = 0;
static int gLowLight_AF = 0;
...
static int isx005_set_metering(struct v4l2_subdev *sd,
...
static int isx005_set_white_balance(struct v4l2_subdev *sd,
				struct v4l2_control *ctrl);
...
enum isx005_frame_size {
	...
	ISX005_PREVIEW_WVGA,		/* 800x480 */
        ISX005_PREVIEW_720P,            /* 1280x720*/
	ISX005_CAPTURE_VGA, /* 640 x 480 */	
	ISX005_CAPTURE_SXGA, /* SXGA  - 1280 x 960 */	
	ISX005_CAPTURE_2MP, /* UXGA  - 1600 x 1200 */
	ISX005_CAPTURE_3MP, /* QXGA  - 2048 x 1536 */
};
...
static struct isx005_enum_framesize isx005_framesize_list[] = {
	...
	{ ISX005_OPRMODE_VIDEO, ISX005_PREVIEW_720P,      1280,  720 },
	{ ISX005_OPRMODE_IMAGE, ISX005_CAPTURE_VGA,	   640,  480 },
	{ ISX005_OPRMODE_IMAGE, ISX005_CAPTURE_SXGA,	  1280,  960 },	
	...
	unsigned short subaddr, unsigned short *data)
{
	...
	if (unlikely(err < 0))
		dev_err(&client->dev, "%s: register read fail\n", __func__);	
...
static inline int isx005_i2c_write_multi(struct i2c_client *client, unsigned short addr, unsigned int w_data, unsigned int w_len)
{
	...
	while(retry_count--){
		...
		if (likely(ret == 1))
			break;
		msleep(POLL_TIME_MS);
		dev_err(&client->dev,"retry : i2c_failed = %d\n", ret);
		}
...
static int isx005_set_frame_rate(struct v4l2_subdev *sd, int fps)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	{
		case 0:
			isx005_msg(&client->dev, " fps 0 means auto_fps.\n");
			break;
			
		case 7:
			err = isx005_write_regs(sd, isx005_7fps,
					sizeof(isx005_7fps) / sizeof(isx005_7fps[0]), "isx005_7fps");
			break;
		case 10:
			err = isx005_write_regs(sd, isx005_10fps,
					sizeof(isx005_10fps) / sizeof(isx005_10fps[0]), "isx005_10fps");
			break;
		case 12:
			err = isx005_write_regs(sd, isx005_12fps,
					sizeof(isx005_12fps) / sizeof(isx005_12fps[0]), "isx005_12fps");
			break;
		case 15:
			err = isx005_write_regs(sd, isx005_15fps,
					sizeof(isx005_15fps) / sizeof(isx005_15fps[0]), "isx005_15fps");
			break;
		case 30:
			err = isx005_write_regs(sd, isx005_30fps,
	...
	//	err = isx005_i2c_write_multi(client, 0x0060, 0x0018, 1); /* UI Brightness adjust */ 
	
		/* camcorder on */
		err = isx005_write_regs(sd, isx005_camcorder_on, sizeof(isx005_camcorder_on) / sizeof(isx005_camcorder_on[0]), "isx005_camcorder_on");
	...
	{
		/* camcorder off */
		err = isx005_write_regs(sd, isx005_camcorder_off, sizeof(isx005_camcorder_off) / sizeof(isx005_camcorder_off[0]), "isx005_camcorder_off");
	}
...
{
//PCAM	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
//PCAM no need	isx005_i2c_write_multi(client, 0x0011, 0x0011, 1);
	return 0;
}
...
static int isx005_set_dzoom(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	default:
		...
		break;
	        case 8:
		err = isx005_write_regs(sd, isx005_Zoom_08, sizeof(isx005_Zoom_08) / sizeof(isx005_Zoom_08[0]),
			"isx005_Zoom_08");
		break;		
	        case 21:
		err = isx005_write_regs(sd, isx005_Zoom_1280_01, sizeof(isx005_Zoom_1280_01) / sizeof(isx005_Zoom_1280_01[0]),
			"isx005_Zoom_1280_01");
		break;
	        case 22:
		err = isx005_write_regs(sd, isx005_Zoom_1280_02, sizeof(isx005_Zoom_1280_02) / sizeof(isx005_Zoom_1280_02[0]),
			"isx005_Zoom_1280_02");
		break;
	        case 23:
		err = isx005_write_regs(sd, isx005_Zoom_1280_03, sizeof(isx005_Zoom_1280_03) / sizeof(isx005_Zoom_1280_03[0]),
			"isx005_Zoom_1280_03");
		break;
	        case 24:
		err = isx005_write_regs(sd, isx005_Zoom_1280_04, sizeof(isx005_Zoom_1280_04) / sizeof(isx005_Zoom_1280_04[0]),
			"isx005_Zoom_1280_04");
		break;
	        case 25:
		err = isx005_write_regs(sd, isx005_Zoom_1280_05, sizeof(isx005_Zoom_1280_05) / sizeof(isx005_Zoom_1280_05[0]),
			"isx005_Zoom_1280_05");
		break;		
	        case 26:
		err = isx005_write_regs(sd, isx005_Zoom_1280_06, sizeof(isx005_Zoom_1280_06) / sizeof(isx005_Zoom_1280_06[0]),
			"isx005_Zoom_1280_06");
		break;		
	        case 31:
		err = isx005_write_regs(sd, isx005_Zoom_1600_01, sizeof(isx005_Zoom_1600_01) / sizeof(isx005_Zoom_1600_01[0]),
			"isx005_Zoom_1600_01");
		break;		
	        case 32:
		err = isx005_write_regs(sd, isx005_Zoom_1600_02, sizeof(isx005_Zoom_1600_02) / sizeof(isx005_Zoom_1600_02[0]),
			"isx005_Zoom_1600_02");
		break;		
	}
...
static int isx005_set_preview_size(struct v4l2_subdev *sd)
{
	...
	case ISX005_PREVIEW_D1:
		...
		err = isx005_i2c_write_multi(client, 0x0028, 0x01E0, 2);// VSIZE_MONI - 480	
		break;
        case ISX005_PREVIEW_720P:
		err = isx005_i2c_write_multi(client, 0x0022, 0x0500, 2);// HSIZE_MONI - 1280
		err = isx005_i2c_write_multi(client, 0x0028, 0x02D0, 2);// VSIZE_MONI - 720	
                break;
	default:
...
static int isx005_set_preview_start(struct v4l2_subdev *sd)
{
	...
	unsigned short read_value = 0x0;
	int i = 0; /* hs43.jeon from P1 src. 20100914 */
...
//		printk( "pattern on setting~~~~~~~~~~~~~~\n");
		...
		msleep(1200); /* 300 -> 900 -> 1200 */
//		printk( "pattern on setting done~~~~~~~~~~~~~~\n");

	}
	else
	{
#if 1 /* Init delay removed */
		if(camera_init == 0) 
		{
			camera_init = 1;
			timeout_cnt = 0;
...
static int isx005_set_capture_size(struct v4l2_subdev *sd)
{
	...
	case ISX005_CAPTURE_VGA: /* 640x480 */
		...
		break;
	case ISX005_CAPTURE_SXGA: /* 1280x960 */
		err = isx005_i2c_write_multi(client, 0x0024, 0x0500, 2); //HSIZE_CAP 1280
	...
	isx005_msg(&client->dev,"isx005_set_capture_size: %d\n", index);
	return 0;	
}
...
static unsigned int isx005_iso_table[] = {25, 32, 40, 50, 64, 80, 100, 125, 160, 200, 250, 320, 400, 500, 640, 800, 1000, 1250, 1600};
static int isx005_get_snapshot_data(struct v4l2_subdev *sd)
{
	...
	if(state->jpeg.enable){
		...
		err = isx005_i2c_read(client, 0x00F0, &temp);
		temp = temp & 0xff; 

		if (temp > 19)
			state->iso_sens_out = isx005_iso_table[18]; /* Array has maximum 19 members */
		else
			state->iso_sens_out = isx005_iso_table[temp - 1];

		isx005_msg(&client->dev, "%s: ISO sensitivity = 0x%x \n", __func__, temp);
		
		/* Shutter speed = shutter_speed_H << 16 + shutter_speed_L */
		err = isx005_i2c_read(client, 0x00f4, &temp);
		state->shutter_speed = temp << 16;
		err = isx005_i2c_read(client, 0x00f2, &temp);
		state->shutter_speed |= temp;
		isx005_msg(&client->dev, "%s: Shutter speed = 0x%x \n", __func__, state->shutter_speed);
...
static int isx005_get_LowLightCondition_AF(struct v4l2_subdev *sd, int *Result)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);	
	int err = 0;
	unsigned short read_value = 0;
...
static int isx005_set_AF_threshold_reset(struct v4l2_subdev *sd)
{
...
static int isx005_set_capture_start(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	/*
	 *  setting capture number to one by default, in low light it will be changed to 3
	 */
	isx005_i2c_write_multi(client, 0x0014, 0x0002, 1);
	...
	if ((read_value & 0xFF) == 0x01) {
		err = isx005_write_regs(sd, isx005_Outdoor_On, sizeof(isx005_Outdoor_On) / sizeof(isx005_Outdoor_On[0]),
			"isx005_Outdoor_On");		
	}
	...
	if (gLowLight) {
//		isx005_i2c_write_multi(client, 0x0014, 0x0003, 1); /* change cap no to 3 */
		if (gCurrentScene == SCENE_MODE_NIGHTSHOT) {
			err = isx005_write_regs(sd, isx005_Night_Mode_On, sizeof(isx005_Night_Mode_On) / sizeof(isx005_Night_Mode_On[0]),
				"isx005_Night_Mode_On");	
		} else {
			err = isx005_write_regs(sd, isx005_Low_Cap_On, sizeof(isx005_Low_Cap_On) / sizeof(isx005_Low_Cap_On[0]),
				"isx005_Low_Cap_On");
		}
	}
	...
	isx005_set_AF_threshold_reset(sd); /* 20101020 */
...
static int isx005_change_scene_mode(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case SCENE_MODE_NONE:
		err = isx005_write_regs(sd, isx005_Scene_Default, sizeof(isx005_Scene_Default) / sizeof(isx005_Scene_Default[0]),
			"isx005_Scene_Default");			
		break;
	case SCENE_MODE_PORTRAIT:
		err = isx005_write_regs(sd, isx005_Scene_Portrait, sizeof(isx005_Scene_Portrait) / sizeof(isx005_Scene_Portrait[0]),
			"isx005_Scene_Portrait");			
		break;
	case SCENE_MODE_NIGHTSHOT:
		err = isx005_write_regs(sd, isx005_Scene_Nightshot, sizeof(isx005_Scene_Nightshot) / sizeof(isx005_Scene_Nightshot[0]),
			"isx005_Scene_Nightshot");			
		break;
	case SCENE_MODE_BACK_LIGHT:
		err = isx005_write_regs(sd, isx005_Scene_Backlight, sizeof(isx005_Scene_Backlight) / sizeof(isx005_Scene_Backlight[0]),
			"isx005_Scene_Backlight");			
		break;
	case SCENE_MODE_LANDSCAPE:
		err = isx005_write_regs(sd, isx005_Scene_Landscape, sizeof(isx005_Scene_Landscape) / sizeof(isx005_Scene_Landscape[0]),
			"isx005_Scene_Landscape");			
		break;
	case SCENE_MODE_SPORTS:
		err = isx005_write_regs(sd, isx005_Scene_Sports, sizeof(isx005_Scene_Sports) / sizeof(isx005_Scene_Sports[0]),
			"isx005_Scene_Sports");			
		break;
	case SCENE_MODE_PARTY_INDOOR:
		err = isx005_write_regs(sd, isx005_Scene_Party_Indoor, sizeof(isx005_Scene_Party_Indoor) / sizeof(isx005_Scene_Party_Indoor[0]),
			"isx005_Scene_Party_Indoor");			
		break;
	case SCENE_MODE_BEACH_SNOW:
		err = isx005_write_regs(sd, isx005_Scene_Beach_Snow, sizeof(isx005_Scene_Beach_Snow) / sizeof(isx005_Scene_Beach_Snow[0]),
			"isx005_Scene_Beach_Snow");			
		break;
	case SCENE_MODE_SUNSET:
		err = isx005_write_regs(sd, isx005_Scene_Sunset, sizeof(isx005_Scene_Sunset) / sizeof(isx005_Scene_Sunset[0]),
			"isx005_Scene_Sunset");			
		break;
	case SCENE_MODE_DUST_DAWN:
		err = isx005_write_regs(sd, isx005_Scene_Duskdawn, sizeof(isx005_Scene_Duskdawn) / sizeof(isx005_Scene_Duskdawn[0]),
			"isx005_Scene_Duskdawn");			
		break;
	case SCENE_MODE_FALL_COLOR:
		err = isx005_write_regs(sd, isx005_Scene_Fall_Color, sizeof(isx005_Scene_Fall_Color) / sizeof(isx005_Scene_Fall_Color[0]),
			"isx005_Scene_Fall_Color");			
		break;
	case SCENE_MODE_FIREWORKS:
		err = isx005_write_regs(sd, isx005_Scene_Fireworks, sizeof(isx005_Scene_Fireworks) / sizeof(isx005_Scene_Fireworks[0]),
			"isx005_Scene_Fireworks");			
		break;		
	case SCENE_MODE_TEXT:
		err = isx005_write_regs(sd, isx005_Scene_Text, sizeof(isx005_Scene_Text) / sizeof(isx005_Scene_Text[0]),
			"isx005_Scene_Text");			
		break;	
	case SCENE_MODE_CANDLE_LIGHT:
		err = isx005_write_regs(sd, isx005_Scene_Candle_Light, sizeof(isx005_Scene_Candle_Light) / sizeof(isx005_Scene_Candle_Light[0]),
...
static int isx005_set_saturation(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case SATURATION_MINUS_2:
		err = isx005_write_regs(sd, isx005_Saturation_Minus_2, sizeof(isx005_Saturation_Minus_2) / sizeof(isx005_Saturation_Minus_2[0]),
			"isx005_Saturation_Minus_2");	
		break;
	case SATURATION_MINUS_1:
		err = isx005_write_regs(sd, isx005_Saturation_Minus_1, sizeof(isx005_Saturation_Minus_1) / sizeof(isx005_Saturation_Minus_1[0]),
			"isx005_Saturation_Minus_1");	
		break;
	case SATURATION_DEFAULT:
	default:
		err = isx005_write_regs(sd, isx005_Saturation_Default, sizeof(isx005_Saturation_Default) / sizeof(isx005_Saturation_Default[0]),
			"isx005_Saturation_Default");	
		break;
	case SATURATION_PLUS_1:
		err = isx005_write_regs(sd, isx005_Saturation_Plus_1, sizeof(isx005_Saturation_Plus_1) / sizeof(isx005_Saturation_Plus_1[0]),
			"isx005_Saturation_Plus_1");	
		break;
	case SATURATION_PLUS_2:
		err = isx005_write_regs(sd, isx005_Saturation_Plus_2, sizeof(isx005_Saturation_Plus_2) / sizeof(isx005_Saturation_Plus_2[0]),
...
static int isx005_set_contrast(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case CONTRAST_MINUS_2:
		err = isx005_write_regs(sd, isx005_Contrast_Minus_2, sizeof(isx005_Contrast_Minus_2) / sizeof(isx005_Contrast_Minus_2[0]),
			"isx005_Contrast_Minus_2");	
		break;
	case CONTRAST_MINUS_1:
		err = isx005_write_regs(sd, isx005_Contrast_Minus_1, sizeof(isx005_Contrast_Minus_1) / sizeof(isx005_Contrast_Minus_1[0]),
			"isx005_Contrast_Minus_1");	
		break;
	case CONTRAST_DEFAULT:
	default:
		err = isx005_write_regs(sd, isx005_Contrast_Default, sizeof(isx005_Contrast_Default) / sizeof(isx005_Contrast_Default[0]),
			"isx005_Contrast_Default");	
		break;
	case CONTRAST_PLUS_1:
		err = isx005_write_regs(sd, isx005_Contrast_Plus_1, sizeof(isx005_Contrast_Plus_1) / sizeof(isx005_Contrast_Plus_1[0]),
			"isx005_Contrast_Plus_1");	
		break;
	case CONTRAST_PLUS_2:
		err = isx005_write_regs(sd, isx005_Contrast_Plus_2, sizeof(isx005_Contrast_Plus_2) / sizeof(isx005_Contrast_Plus_2[0]),
...
static int isx005_set_sharpness(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case SHARPNESS_MINUS_2:
		err = isx005_write_regs(sd, isx005_Sharpness_Minus_2, sizeof(isx005_Sharpness_Minus_2) / sizeof(isx005_Sharpness_Minus_2[0]),
			"isx005_Sharpness_Minus_2");	
		break;
	case SHARPNESS_MINUS_1:
		err = isx005_write_regs(sd, isx005_Sharpness_Minus_1, sizeof(isx005_Sharpness_Minus_1) / sizeof(isx005_Sharpness_Minus_1[0]),
			"isx005_Sharpness_Minus_1");	
		break;
	case SHARPNESS_DEFAULT:
	default:
		err = isx005_write_regs(sd, isx005_Sharpness_Default, sizeof(isx005_Sharpness_Default) / sizeof(isx005_Sharpness_Default[0]),
			"isx005_Sharpness_Default");	
		break;
	case SHARPNESS_PLUS_1:
		err = isx005_write_regs(sd, isx005_Sharpness_Plus_1, sizeof(isx005_Sharpness_Plus_1) / sizeof(isx005_Sharpness_Plus_1[0]),
			"isx005_Sharpness_Plus_1");	
		break;
	case SHARPNESS_PLUS_2:
		err = isx005_write_regs(sd, isx005_Sharpness_Plus_2, sizeof(isx005_Sharpness_Plus_2) / sizeof(isx005_Sharpness_Plus_2[0]),
...
static int isx005_set_white_balance(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case WHITE_BALANCE_AUTO:
		err = isx005_write_regs(sd, isx005_WB_Auto, sizeof(isx005_WB_Auto) / sizeof(isx005_WB_Auto[0]),
			"isx005_WB_Auto");
		break;
	case WHITE_BALANCE_SUNNY:
		err = isx005_write_regs(sd, isx005_WB_Sunny, sizeof(isx005_WB_Sunny) / sizeof(isx005_WB_Sunny[0]),
			"isx005_WB_Sunny");
		break;
	case WHITE_BALANCE_CLOUDY:
		err = isx005_write_regs(sd, isx005_WB_Cloudy, sizeof(isx005_WB_Cloudy) / sizeof(isx005_WB_Cloudy[0]),
			"isx005_WB_Cloudy");
		break;
	case WHITE_BALANCE_TUNGSTEN:
		err = isx005_write_regs(sd, isx005_WB_Tungsten, sizeof(isx005_WB_Tungsten) / sizeof(isx005_WB_Tungsten[0]),
			"isx005_WB_Tungsten");
		break;
	case WHITE_BALANCE_FLUORESCENT:
		err = isx005_write_regs(sd, isx005_WB_Fluorescent, sizeof(isx005_WB_Fluorescent) / sizeof(isx005_WB_Fluorescent[0]),
...
static int isx005_set_ev(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch (ctrl->value) {
	case EV_MINUS_4:
		err = isx005_write_regs(sd, isx005_EV_Minus_4, sizeof(isx005_EV_Minus_4) / sizeof(isx005_EV_Minus_4[0]),
			"isx005_EV_Minus_4");
		break;
	case EV_MINUS_3:
		err = isx005_write_regs(sd, isx005_EV_Minus_3, sizeof(isx005_EV_Minus_3) / sizeof(isx005_EV_Minus_3[0]),
			"isx005_EV_Minus_3");
		break;
	case EV_MINUS_2:
		err = isx005_write_regs(sd, isx005_EV_Minus_2, sizeof(isx005_EV_Minus_2) / sizeof(isx005_EV_Minus_2[0]),
			"isx005_EV_Minus_2");
		break;
	case EV_MINUS_1:
		err = isx005_write_regs(sd, isx005_EV_Minus_1, sizeof(isx005_EV_Minus_1) / sizeof(isx005_EV_Minus_1[0]),
			"isx005_EV_Minus_1");
		break;
	case EV_DEFAULT:
		err = isx005_write_regs(sd, isx005_EV_Default, sizeof(isx005_EV_Default) / sizeof(isx005_EV_Default[0]),
			"isx005_EV_Default");
		break;
	case EV_PLUS_1:
		err = isx005_write_regs(sd, isx005_EV_Plus_1, sizeof(isx005_EV_Plus_1) / sizeof(isx005_EV_Plus_1[0]),
			"isx005_EV_Default");
		break;
	case EV_PLUS_2:
		err = isx005_write_regs(sd, isx005_EV_Plus_2, sizeof(isx005_EV_Plus_2) / sizeof(isx005_EV_Plus_2[0]),
			"isx005_EV_Plus_2");
		break;
	case EV_PLUS_3:
		err = isx005_write_regs(sd, isx005_EV_Plus_3, sizeof(isx005_EV_Plus_3) / sizeof(isx005_EV_Plus_3[0]),
			"isx005_EV_Plus_3");
		break;
	case EV_PLUS_4:
		err = isx005_write_regs(sd, isx005_EV_Plus_4, sizeof(isx005_EV_Plus_4) / sizeof(isx005_EV_Plus_4[0]),
			"isx005_EV_Plus_4");
		break;			
	default:
...
static int isx005_set_metering(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case METERING_MATRIX:
		err = isx005_write_regs(sd, isx005_Metering_Matrix, sizeof(isx005_Metering_Matrix) / sizeof(isx005_Metering_Matrix[0]),
			"isx005_Metering_Matrix");
		break;
	case METERING_CENTER:
		err = isx005_write_regs(sd, isx005_Metering_Center, sizeof(isx005_Metering_Center) / sizeof(isx005_Metering_Center[0]),
			"isx005_Metering_Center");
		break;
	case METERING_SPOT:
		err = isx005_write_regs(sd, isx005_Metering_Spot, sizeof(isx005_Metering_Spot) / sizeof(isx005_Metering_Spot[0]),
...
static int isx005_set_iso(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case ISO_AUTO:
	default:
		err = isx005_write_regs(sd, isx005_ISO_Auto, sizeof(isx005_ISO_Auto) / sizeof(isx005_ISO_Auto[0]),
			"isx005_ISO_Auto");
		break;
	case ISO_100:
		err = isx005_write_regs(sd, isx005_ISO_100, sizeof(isx005_ISO_100) / sizeof(isx005_ISO_100[0]),
			"isx005_ISO_100");
		break;
	case ISO_200:
		err = isx005_write_regs(sd, isx005_ISO_200, sizeof(isx005_ISO_200) / sizeof(isx005_ISO_200[0]),
			"isx005_ISO_200");
		break;
	case ISO_400:
		err = isx005_write_regs(sd, isx005_ISO_400, sizeof(isx005_ISO_400) / sizeof(isx005_ISO_400[0]),
			"isx005_ISO_400");
		break;
...
#if 1 /* 20101020 */
		isx005_get_LowLightCondition_AF(sd, &gLowLight_AF);
		if(gLowLight_AF)
		{
...
static int isx005_get_auto_focus_status(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if ((read_value & 0xFF) == 0x01) {
		...
		stop_af_operation = 1; /* For AF Macro mode */
		ctrl->value = 0x00;	/* 0x00 --> AF failed*/ 
	}
...
static void isx005_init_parameters(struct v4l2_subdev *sd)
{
	...
	state->strm.parm.capture.timeperframe.denominator = 30;	/* Default value */
	state->strm.parm.capture.capturemode = 0;

	/* set the index only if it is not set before */
	if (!state->framesize_index)
		state->framesize_index = ISX005_PREVIEW_VGA;
...
static int isx005_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *param)
{
	...
	if(state->runmode != ISX005_RUNMODE_RUNNING && state->fps != 0){
		isx005_info(&client->dev, "frame rate = %d\n",state->fps);
		isx005_set_frame_rate(sd,state->fps);
	}
	return err;
}
...
static int isx005_get_framesize_index(struct v4l2_subdev *sd)
{
	...
	return (state->oprmode == ISX005_OPRMODE_IMAGE) ? ISX005_CAPTURE_3MP : ISX005_PREVIEW_VGA;
}
...
#if 0  /* hs43.jeon from P1 src */
	case V4L2_CID_CAMERA_FLASH_CHECK:
#if 0 /* Palladio has no flash, isn't it? */
...
static int dnie_update_fw(struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
    struct isx005_state *state = to_state(sd);
	...
	for(j = 0; j < fw_size[0]; j++){
			printk("dnie_update_fw: , fw_size[0]: %d, test[0]: 0x%x\n", fw_size[0], test[0][j]);
			test[0][j] = ((test[0][j]&0xff00)>>8)|((test[0][j]&0x00ff)<<8);
			printk("dnie_update_fw: , test1[0]: 0x%x\n", test[0][j]);				
		}
...
static int isx005_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	isx005_msg(&client->dev, "%s: V4l2 control ID =%d( value = %d)\n", __func__, ctrl->id - V4L2_CID_PRIVATE_BASE,ctrl->value);

#if 1 /* hs43.jeon 20100914 */
...
/*Camcorder fix fps*/
	...
	case V4L2_CID_CAMERA_WDR://unsupported
		err = 0;
		break;

	case V4L2_CID_CAMERA_ANTI_SHAKE://unsupported
		err = 0;
		break;

	case V4L2_CID_CAMERA_FACE_DETECTION://unsupported
		err = 0;
		break;

	case V4L2_CID_CAMERA_SMART_AUTO://unsupported
		err = 0;
		break;
	...
	case V4L2_CID_CAMERA_VINTAGE_MODE://unsupported
		err = 0;
		break;
		
	case V4L2_CID_CAMERA_BEAUTY_SHOT://unsupported
		err = 0;
		break;

	case V4L2_CID_CAMERA_FACEDETECT_LOCKUNLOCK://unsupported
		err = 0;
		break;		
...
//		err = isx005_write_regs(sd, isx005_vt_15fps,sizeof(isx005_vt_15fps) / sizeof(isx005_vt_15fps), "isx005_vt_15fps");
...
{	
	...
	isx005_info(&client->dev,"isx005_calibration : start \n");
	...
	isx005_info(&client->dev,"%s [cam] CALIBRATION : END \n", __func__); 
	return 0;		
}
...
#if 1 /* hs43.jeon from P1 src. for check_dataline */
...
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int err = -EINVAL, i, count=0,index;
	...
	isx005_info(&client->dev, "%s: init setting~~~~~~~~~~~~~~\n", __func__);
	//msleep(5); /* from P1, hs43.jeon */
	gpio_direction_output(GPIO_CAM_VGA_nSTBY, 0); /* standby pin_low */
	udelay(500);
...
//PCAM	msleep(10);	
	...
	gpio_direction_output(GPIO_CAM_VGA_nSTBY, 1); /* standby pin_high */
	msleep(15); /* wait stdby mode */
...
//	isx005_cam_stdby(0);//standby pin
#else
	//for(i = 0; i <ISX005_INIT_REGS; i++)
	for(i = ISX005_INIT_REGS; i>0;i--)
	{
		index = ISX005_INIT_REGS - i;
		err = isx005_i2c_write_multi(client, isx005_init_reg[index].subaddr ,  isx005_init_reg[index].value,  isx005_init_reg[index].len);
		if (err < 0)
		{
			isx005_msg(&client->dev, "%s: register read fail \n", __func__);	
			return -EIO;
...
#if 1 /* from P1 src. hs43.jeon */
	isx005_info(&client->dev,"standby enable\n");
	gpio_direction_output(GPIO_CAM_VGA_nSTBY, 1); /* standby pin_high */
	mdelay(15); //msleep(15); /* wait stdby mode */
#endif
	...
	isx005_info(&client->dev, "%s: isx005_init_image_tuning_setting~~~~~~~~~~~~~~\n", __func__);		
	//for(i = 0; i <ISX005_INIT_IMAGETUNING_SETTING_REGS; i++)
	for(i=ISX005_INIT_IMAGETUNING_SETTING_REGS; i>0;i--)
	{
		index = ISX005_INIT_IMAGETUNING_SETTING_REGS - i;
		err = isx005_i2c_write_multi(client, isx005_init_image_tuning_setting[index].subaddr ,  isx005_init_image_tuning_setting[index].value ,  isx005_init_image_tuning_setting[index].len);
		if (err < 0)
		{
			isx005_msg(&client->dev, "%s: register read fail \n", __func__);	
			return -EIO;
