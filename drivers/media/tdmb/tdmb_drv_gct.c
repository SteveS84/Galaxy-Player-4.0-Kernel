...
// DBG
//#define TDMB_DEBUG
#define TDMB_KERN_DEBUG KERN_DEBUG
...
void TDMB_drv_gct_download()
{
	...
	kfree(verify_buf);
	printk(TDMB_KERN_DEBUG "[%s:%d] Binary verification success!! \n", __FUNCTION__, __LINE__);
#endif
...
#define TEST_MM_READ_SIZE (188*15)
  ...
  if(g_gct_mm_read_buf == NULL || g_gct_burst_read_buf == NULL)
  {
	...
	printk(TDMB_KERN_DEBUG "[%s] alloc error \n", __FUNCTION__);
    return -2;
  }
...
#define DUMP_FILE_NAME "/system/ts_data.ts"
	...
	if(fp_ts_data == NULL) 
	{
		...
		if(fp_ts_data == NULL) 
		{
			printk(TDMB_KERN_DEBUG "[%s] file open error!\n", __FUNCTION__);
			return;
		}
	}
	...
	//printk(TDMB_KERN_DEBUG "[%s] save ts data \n", __FUNCTION__);
    ...
    if(bfirst)
    {
        for(j=0;j<dwDataSize-2;j++)
        {
            ...
            {                
                ...
                printk(TDMB_KERN_DEBUG "!!!!!!!!!!!!! first sync=%d !!!!!!!!!!!!",mp2len);
                if(mp2len<0)
                    return -1;
            ...
            while(MSCBuffpos>readpos)
            {
                ...
                if(first)
                {
                    ...
                    if(!(MSCBuff[0]==0xFF && (MSCBuff[1]>>4==0xF)))
                    {
                        ...
                        MSCBuffpos =0;
                        bfirst =1;
                        printk(TDMB_KERN_DEBUG "!!!!!!!!!!!!! error 0x%x,0x%x!!!!!!!!!!!!",MSCBuff[0], MSCBuff[1]);
                        return -1;
                    }
...
void gdm_hal_read_msc( void )
{
  ...
  {
	  bnon = 1;
	//printk(TDMB_KERN_DEBUG "### [%s] res : %d\n",__FUNCTION__, gdm_read_test2());
	  gdm_test();
	return;
  }
#endif
...
//		  usCurrentInterrupt, usInterruptMask, usInterruptStatus); 
...
#endif
  {
      ...
      {
		printk(TDMB_KERN_DEBUG " INT_DAB_MSC_OV #######\n");
