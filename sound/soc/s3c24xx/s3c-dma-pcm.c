	...
	struct snd_pcm_hw_params *params)
{
	...
	pr_debug("[PCM] DmaAddr=@%x Total=%lubytes PrdSz=%u #Prds=%u dma_area=0x%x\n",
...
struct snd_soc_platform s3c24xx_pcm_soc_platform = {
	.name		= "s3c24xx-pcm-audio",
	.pcm_ops	= &s3c_dma_ops,
	.pcm_new	= s3c_dma_new,
	.pcm_free	= s3c_dma_free_dma_buffers,
};
EXPORT_SYMBOL_GPL(s3c24xx_pcm_soc_platform);

static int __init s3c24xx_soc_platform_init(void)
{
	return snd_soc_register_platform(&s3c24xx_pcm_soc_platform);
}
module_init(s3c24xx_soc_platform_init);

static void __exit s3c24xx_soc_platform_exit(void)
{
	snd_soc_unregister_platform(&s3c24xx_pcm_soc_platform);
}
