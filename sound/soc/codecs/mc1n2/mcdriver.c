...
static SINT32	set_digitalio			(MCDRV_DIO_INFO* psDioInfo, UINT32 dUpdateInfo);   
...
)
{
	...
	MCDRV_POWER_UPDATE	sPowerUpdate;
printk(KERN_DEBUG "\n *******Yosef:read_reg:psRegInfo->bRegType=  %d, abData[1]= %d  \n",psRegInfo->bRegType,bAddr);   //yosef add		
	if(NULL == psRegInfo)
	{
		return MCDRV_ERROR_ARGUMENT;
	}
...
printk(KERN_DEBUG "\n *******Yosef:read_reg:MCDRV_REGTYPE_B_ANALOG:psRegInfo->bRegType=  %d, abData[1]= %d  \n",psRegInfo->bRegType,bAddr);   //yosef add			
...
printk(KERN_DEBUG "\n *******Yosef:read_reg:McSrv_ReadI2C:bSlaveAddr=%d, bAddr=%d, psRegInfo->bData=  %d \n",bSlaveAddr, bAddr,psRegInfo->bData);   //yosef add	
...
)
{
	...
	if(eMCDRV_STATE_NOTINIT == eState)
	{
	 ...
	 #if 1
// Yamaha_HDMI  yosef
	psDioInfo->asPortInfo[0].sDioCommon.bMasterSlave = MCDRV_DIO_MASTER;
	psDioInfo->asPortInfo[0].sDioCommon.bAutoFs = MCDRV_AUTOFS_ON;
	psDioInfo->asPortInfo[0].sDioCommon.bFs = MCDRV_FS_44100;
	psDioInfo->asPortInfo[0].sDioCommon.bBckFs = MCDRV_BCKFS_32;
	psDioInfo->asPortInfo[0].sDioCommon.bInterface = MCDRV_DIO_DA;
// Yamaha_HDMI
#endif
...
//            printk(KERN_DEBUG "\n Yosef:MCDRV_SET_VOLUME \n");   //yosef add				
			...
			case	MCDRV_SET_SYSEQ:
				...
				break;
			case	MCDRV_READ_REG :
	printk(KERN_DEBUG "\n *******Yosef:McDrv_Ctrl:MCDRV_READ_REG \n");   //yosef add
