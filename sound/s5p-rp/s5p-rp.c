...
#define _WBUF_SIZE_		(256*1024)	/* in DRAM */
...
struct s5p_rp_info {
	...
	unsigned long am_filter_xchg_addr;	/* AM filter exchanginng addr */
...
static void s5p_rp_am_filter_download(void)
{
        unsigned long n;
        unsigned long *pval;

        /* AM filter table in DMEM */
        s5pdbg("AM filter downloaded\n");
        for (n = s5p_rp.am_filter_xchg_addr, pval = (unsigned long *)s5p_rp.am_filter;
                 n < s5p_rp.am_filter_xchg_addr + 60; n += 4, pval++) {
                writel(*pval, s5p_rp.dmem + n);
        }
}
...
static void s5p_rp_reset(void)
{
	...
	writel(0x00000000, s5p_rp.commbox + RP_FRAME_INDEX);			/* Clear Decoded Frame No. = 0 */
	writel(0x00000000, s5p_rp.commbox + RP_READ_BITSTREAM_SIZE);
	writel(_BITSTREAM_SIZE_MAX_, s5p_rp.commbox + RP_BITSTREAM_SIZE);			/* Set Total Bitstream Size to Maximum */
...
static int s5p_rp_choose_fw_set(void)
{
	...
	case SA_SUROUND:			/* m-Theater */
		...
		break;
	case SA_3D:				/* 3D (S1/P1) */
/*	case SA_VOCAL:				   Vocal (Palladio) */
	case SA_EQ:				/* EQ (Palladio) */
		fw_set_new = RP_FW_CODE20;
		ret_val = 1;			/* Vocal/EQ share CODE20, force change-flag ON */
		break;
	...
	case SA_BE:				/* Bass Enhancement */
		...
		break;
	case SA_SE:				/* Sound Externalization (S1/P1) */
/*	case SA_CAFE:				   Cafe (Palladio) */
...
{
	unsigned long sw_def = readl(s5p_rp.commbox + RP_SW_DEF);
	unsigned long effect_def;

	effect_def = s5p_rp.effect_def | s5p_rp.effect_speaker;
	effect_def &= ~(0x01 << 20);
	effect_def |= s5p_rp.force_mono_enabled ? 0x01 << 20 : 0;

	writel(effect_def, s5p_rp.commbox + RP_EFFECT_DEF);
...
static void s5p_rp_set_force_mono_apply(void)
{
	unsigned long effect_def = readl(s5p_rp.commbox + RP_EFFECT_DEF);
...
static void s5p_rp_setup_timeout_eos(void)
{
	...
	remaining_msec += (_OBUF_SIZE_ * 2) * 1000 / 44100 / 4;
...
static irqreturn_t s5p_rp_irq(int irqno, void *dev_id)
{
	...
	if (irq_code & RP_INTR_CODE_EFFECTDATA_ADDR) {
		s5pdbg("Effect data addr [0x%08lx]\n", irq_info);		/* DMEM offset */
		pendingoff_req = 1;
	}

	if (irq_code & RP_INTR_CODE_AMFILTER_ADDR) {
		s5pdbg("AM filter addr [0x%08lx]\n", irq_info);			/* DMEM offset */
		s5p_rp.am_filter_xchg_addr = irq_info;
		if (s5p_rp.am_filter_loaded)
			s5p_rp_am_filter_download();
		pendingoff_req = 1;
	}
...
static int s5p_rp_ctrl_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case S5P_RP_CTRL_EFFECT_DEF:
		...
		s5p_rp.effect_def = arg & 0x1FFFFE;
		if (s5p_rp_is_running) {
			val = s5p_rp.effect_def | s5p_rp.effect_speaker;
			val &= ~(0x01 << 20);
			val |= s5p_rp.force_mono_enabled ? 0x01 << 20 : 0;

			writel(val, s5p_rp.commbox + RP_EFFECT_DEF);
		} else if (s5p_rp_is_opened) {
			s5p_rp_fw_effect_download();			/* Download immediately */
			s5p_rp_set_effect_apply();			/* Apply immediately */
		}
